...
const ChildFood = (p) => {
  ...
  const open = () => Animated.timing(p.anim, {toValue: 80,duration: 150,useNativeDriver: false}).start();
  ...
  useEffect(() => {inputPrice == 0 && close() }, [inputPrice])
  ...
  return (
    ...
    <SearchInput {...p} searcher={searcher} foodAsc={foodAsc} foodDesc={foodDesc}  />
      <Span f={Platform.OS === 'web'?1:.8} class={s.viewContainer} land={{ paddingHorizontal: 8, alignSelf: 'center' }} >
                        ...
                        <Span mt={-2} class={[s.TextPlus]} style={ p.width < 317 ? {minWidth:52}: {minWidth:63}} >
                          { item.available ?
                              <Press dir='rtl' class={[s.viewIcons]} >
                                <Icon size={20} name="plus" style={{ paddingVertical: 5 }} onPress={() => { open(); plus(index, item)}} color='blue' />
                                  <Span mv={2} />
        ...
        <Animated.View f={.2} onStartShouldSetResponder={() => { 
          if(p.tokenValue.fullname)p.navigation.navigate("FinallFoodPayment") 
          else p.navigation.navigate("Login", { name: 'ChildFood', price: inputPrice }) 
          }}
          style={[styles.bottomPayment, { height: p.anim, }]}>
